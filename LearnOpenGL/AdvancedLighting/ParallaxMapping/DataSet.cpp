...
Buffer * InitBrickWallBuffer()
{
	...
	//计算第二个三角形的副切线
	glm::vec3 bitangent1 = glm::cross(glm::vec3(0, 0, 1), tangent1);
	...
	//计算第二个三角形的副切线
	glm::vec3 bitangent2 = glm::cross(glm::vec3(0, 0, 1), tangent2);
	...
	buffer->LoadVertexData((const float *)bitangent, 4, 3);
...
void Update(void *param)
{
	...
	Texture *diffuseMap = drawParam->textures[0];
	Texture *normalMap = drawParam->textures[1];
	Texture *depthMap = drawParam->textures[2];
	...
	//绑定纹理单元
	diffuseMap->BindUnit(shader, "diffuseMap", 0);
	normalMap->BindUnit(shader, "normalMap", 1);
	depthMap->BindUnit(shader, "depthMap", 2);
	//绑定绘制缓存
	buffer->Bind();
	//绘制图元
